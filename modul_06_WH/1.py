...
напиши функцію : Функція отримує один аргумент: шлях до каталогу path_folder .
Функція сканує вміст каталогу, що знаходиться за шляхом path_folder.
Якщо знайдений об'єкт є каталогом, функція перевіряє, чи знаходиться каталог за шляхом path_folder в кореневому каталозі. Якщо так, то він перевіряє, чи назва каталога є однією з п'яти дозволених назв ('images', 'video', 'documents', 'audio', 'archives').
Якщо каталог є порожнім та знаходиться в кореневому каталозі, який не є одним з п'яти дозволених, то функція видаляє його. Якщо каталог містить файли, функція рекурсивно викликає саму себе для кожного каталогу.
Якщо знайдений об'єкт є файлом, функція нічого не робить.
Функція продовжує сканування каталогу, доки не будуть перевірені всі об'єкти.
Функція не повертає значення. Вона просто видаляє порожні каталоги в кореневому каталозі, які не є одними з п'яти дозволених.